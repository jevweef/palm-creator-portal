...
def main(argv) -> int:
    ...
    for r in posts:
        ...
        if "fail" in (f.get("Status") or "").lower():
            ...
            tg_failed.append({"who": who or "?", "title": title[:70], "when": when, "age": age, "id": r["id"]})
    ...
    for x in sorted(tg_failed, key=lambda v: -v["age"]):
        line = (f"{x['who']}'s post from {x['when']} (\"{x['title']}\") never made it to her posting"
                ...
                f" Re-send it from there or delete it. https://app.palm-mgmt.com/admin/social?tab=outbound&sub=postprep&focusPost={x['id']}")
