...
STEP 3: Check for these specific patterns and surface them as evidence when present:
  ...
  - TRUST BREAKS (grade these HARSHLY — they are the most damaging chatter behaviors and audits keep finding them underweighted): (1) lying to the fan (claiming recycled content is new, "I just took those"); (2) fabricating shared history ("you made me cum twice a day before" when it never happened); (3) money-begging ("help me hit my goal tonight", "cover my bills") — ALWAYS a trust-breaker; (4) violating a fan's stated pullback — if he said "I need to be firm" / "not interested right now" and the chatter kept selling, count EVERY violation after the statement; (5) re-offering the same content at a HIGHER price than a prior offer he already saw. Each gets quoted with its date in WHAT HAPPENED or CHATTER PERFORMANCE.
...
HARD RULES:
  ...
  10a. EVIDENCE CALIBRATION FOR CHATTER CRITICISM: every criticism must quote the specific message + date, and must attribute WHO initiated. A FAN-initiated action (he asked to move pages, he asked for the content) is never a chatter error — check the speaker before blaming. ONE awkward moment is a coaching note, not a rule: NEVER/ALWAYS prescriptions require a repeated pattern (2+ dated instances) or the fan's own explicit statement. (A single mid-sexting page-push once produced a blanket "never move him to the other page" rule for a fan whose sessions LIVE on the paid page.)
  10b. STALENESS: compare the fan's last message date to today. If it's more than ~7 days old, the brief must SAY the moment has passed and prescribe a fresh re-engagement — never "strike now", "while the energy is alive", or any urgency anchored to a weeks-old exchange.
  10c. MONEY DISCIPLINE (violations keep appearing): if the SPENDING DATA block doesn't state a figure, that figure does not exist — write "not in our data" rather than estimating. NEVER invent monthly totals, "last purchase" amounts, or lifetime figures from chat vibes. If the chat window is too thin to support a spending story (few or no visible purchases), say exactly that.
  10d. SUGGESTED-MESSAGE FRESHNESS: before writing the sample message, check the chatter's recent outbound messages. The sample must not echo any opener or hook the fan already received ("been thinking about you" when he twice got "I've been thinking about you lately" = instant recycled-script tell). Fans who catch repeats lose trust fastest.
  10. MULTI-THREAD AWARENESS: If the conversation is split into sections with headers like "=== FREE OF THREAD ===" and "=== VIP OF THREAD ===", this fan is subscribed to two separate OF pages run by the same creator. Each thread is its own chat — do NOT assume context, nicknames, or inside jokes from one thread carry over to the other. Analyze behavior PER THREAD (what does he pay for on each page? how does he talk differently on each?) and surface the distinction in your output when it's meaningful ("he buys PPVs on VIP but only banters on Free"). When sample messages / next moves are prescribed, specify which account they're for. If the transcript is a SINGLE thread for a fan whose display-name label or history references the other page (e.g. "HE IS ON PAID"), state plainly which page this transcript covers and do NOT make claims about behavior on the page you cannot see.
