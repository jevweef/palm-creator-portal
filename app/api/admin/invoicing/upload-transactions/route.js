...
function parseDate(dateStr, timeStr) {
  // "Apr 6, 2026" "6:30 pm" → Date object
...
function parseSales(lines) {
  ...
  while (i < lines.length) {
    ...
    if (m) {
      ...
      txns.push({ dt, gross, of_fee, net, type, fan, desc, origDate: null })
      i++
    } else {
      i++
    }
...
function parseChargebacks(lines) {
  ...
  while (i < lines.length) {
    ...
    txns.push({ dt: cbDt, gross, of_fee, net, type, fan, desc, origDate: origDt })
  }
  return txns
}
...
const HEADER_ROW = ['Date', 'Time', 'Gross', 'OF Fee', 'Net', 'Type', 'Fan Username', 'Original Date', 'Description']
...
async function getOrCreateTab(sheets, creator, dataType) {
  ...
  if (!existing) {
    ...
    // Write header + cutoff notice
    await sheets.spreadsheets.values.batchUpdate({
      spreadsheetId: SPREADSHEET_ID,
      resource: {
        valueInputOption: 'RAW',
        data: [
          { range: `'${tabName}'!A1`, values: [['⏳ No data uploaded yet — paste your first batch!']] },
...
async function updateCutoff(sheets, tabName, cutoffDt) {
  const notice = cutoffDt
    ...
    : '⏳ No data uploaded yet — paste your first batch!'
...
export async function POST(request) {
  ...
  const { rawData, creator, dataType } = await request.json()

  if (!rawData || !creator) {
    return Response.json({ error: 'Missing rawData or creator' }, { status: 400 })
  }

  // Parse the raw data
  const lines = rawData.split('\n').map(l => l.trim()).filter(Boolean)
  if (lines.length === 0) {
    return Response.json({ error: 'No data to parse' }, { status: 400 })
  }
  ...
  const txns = fmt === 'chargebacks' ? parseChargebacks(lines) : parseSales(lines)

  if (txns.length === 0) {
    return Response.json({ error: 'Could not parse any transactions from the pasted data' }, { status: 400 })
  }
  ...
  try {
    ...
    const sheetType = fmt === 'chargebacks' ? 'Chargebacks' : 'Sales'
    ...
    for (const t of filtered) {
      ...
      typeBreakdown[tp] = (typeBreakdown[tp] || 0) + t.net
      const fan = t.fan || 'Unknown'
