...
// POST — same shape as Stage B's resolver: find still-Generating rows,
// poll WaveSpeed by the stored Prediction ID, finalize anything done.
export async function POST() {
  try {
    ...
    for (const r of rows.slice(0, BATCH)) {
      ...
      if (status === 'completed') {
        ...
        if (tok && ns) {
          ...
          } catch (e) { console.warn(`[outfit-swap/resolve] dropbox ${r.id}: ${e.message}`) }
        }
    ...
    console.error('[outfit-swap/resolve] error:', msg)
