...
// Default export: the shared AI editor body. Used by two mount points:
//   - /ai-editor (standalone — the historic ai_editor user-role surface)
//   - /admin/recreate-source?tab=workflow (embedded inside the admin shell)
//
// When `embedded` is true, the inner tab state (workspace / create / carousel)
// is held in React state instead of the URL — because the outer admin page
// already uses `?tab=` for its own (setup / workflow / strategy). Writing to
// the URL would yank the outer tab strip back to default.
export default function AiEditorBody({ embedded = false } = {}) {
  ...
  // Inner tab — URL-backed standalone; local-state when embedded.
  const tabParam = embedded ? null : sp.get('tab')
  const [localTab, setLocalTab] = useState('workspace')
  const urlTab = tabParam === 'create' ? 'create' : tabParam === 'carousel' ? 'carousel' : 'workspace'
  ...
  const setTab = (k, extra = {}) => {
    if (embedded) {
      ...
      // URL — those don't collide with the outer admin tab strip.
