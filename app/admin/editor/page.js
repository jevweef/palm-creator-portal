...
function StatusBadge({ status }) {
  const c = STATUS_COLORS[status] || { bg: '#FFF0F3', text: '#999', border: 'transparent' }
  return (
    <span style={{
      ...
      <div style={{ background: 'var(--card-bg-solid)', border: 'none', boxShadow: '0 8px 40px rgba(0,0,0,0.12)', borderRadius: '18px', padding: '24px', width: '440px', maxWidth: '95vw' }}
        ...
        <div style={{ display: 'flex', gap: '8px', marginTop: '16px', justifyContent: 'flex-end' }}>
          <button onClick={onClose} disabled={uploading}
            style={{ padding: '8px 16px', border: 'none', borderRadius: '6px', color: '#1a1a1a', fontSize: '13px', fontWeight: 600, cursor: 'pointer', background: 'transparent' }}>
            Cancel
          </button>
          <button onClick={handleSubmit} disabled={!file || uploading}
            style={{
              ...
              background: !file || uploading ? 'transparent' : '#E88FAC', opacity: uploading ? 0.6 : 1,
            }}>
  ...
  return (
    <div style={{ borderTop: '1px solid transparent', paddingTop: '8px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', marginBottom: '6px' }}>
        {audioType && (
          <span style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '11px', background: 'rgba(232, 160, 160, 0.04)', color: '#888', border: '1px solid transparent' }}>
            {audioType}
          </span>
        )}
        {identifiedLabel ? (
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                border: `1px solid ${filter === tab.key ? '#E88FAC' : 'transparent'}`,
        ...
        <button
          ...
          style={{
            padding: '6px 14px', fontSize: '12px', fontWeight: 600,
            background: 'var(--card-bg-solid)', color: '#888', border: '1px solid transparent',
            borderRadius: '6px', cursor: 'pointer',
          }}
        >
          Refresh
      ...
      {filtered.length === 0 ? (
        <div style={{ padding: '60px', textAlign: 'center', color: '#555', fontSize: '14px', background: 'var(--card-bg-solid)', borderRadius: '18px', border: 'none', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {task.inspo.contentLink && (
            <a href={task.inspo.contentLink} target="_blank" rel="noopener noreferrer"
              style={{ fontSize: '11px', color: '#E88FAC', textDecoration: 'none', padding: '3px 8px', background: 'rgba(232, 160, 160, 0.04)', borderRadius: '4px', border: '1px solid transparent' }}>
              Original Reel ↗
            </a>
          )}
        ...
        <div style={{ marginTop: 'auto', paddingTop: '4px' }}>
          {task.status === 'To Do' && (
            <button onClick={onStartEditing} disabled={updating}
              style={{ width: '100%', padding: '10px', fontSize: '13px', fontWeight: 600, background: updating ? 'transparent' : '#dcfce7', color: updating ? '#555' : '#22c55e', border: '1px solid #bbf7d0', borderRadius: '8px', cursor: 'pointer', opacity: updating ? 0.6 : 1 }}>
              {updating ? 'Updating...' : 'Start Editing'}
            </button>
          )}
          {task.status === 'In Progress' && (
            <button onClick={onSubmit}
              style={{ width: '100%', padding: '10px', fontSize: '13px', fontWeight: 600, background: 'rgba(232, 160, 160, 0.04)', color: '#E88FAC', border: '1px solid #E88FAC', borderRadius: '8px', cursor: 'pointer' }}>
              Submit for Review
            </button>
          )}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <select
            ...
            style={{
              ...
              background: 'var(--background)', color: '#1a1a1a', border: '1px solid transparent',
    ...
    <div style={{
      ...
      <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '10px', flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
          ...
          {asset.sourceType && (
            <span style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 600, background: 'rgba(232, 160, 160, 0.04)', color: '#999', border: '1px solid transparent', whiteSpace: 'nowrap', flexShrink: 0 }}>
              {asset.sourceType}
            </span>
          )}
        </div>
        ...
        {/* Folder path hint */}
        {folderLabel && (
          <div style={{ fontSize: '11px', color: '#555', fontFamily: 'monospace', background: 'var(--background)', padding: '4px 8px', borderRadius: '4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            📁 {folderLabel}
          </div>
        )}
            ...
            <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={handleUseFullFrame} disabled={uploading}
                  style={{ flex: 1, padding: '9px', background: 'rgba(232, 160, 160, 0.04)', border: '1px solid transparent', color: '#999', borderRadius: '8px', cursor: uploading ? 'default' : 'pointer', fontSize: '12px', fontWeight: 600, opacity: uploading ? 0.5 : 1 }}>
                  Use full frame
                </button>
      ...
      <div style={{ background: 'var(--card-bg-solid)', border: '1px solid transparent', borderRadius: '16px', padding: '20px', maxWidth: '95vw', display: 'flex', flexDirection: 'column', gap: '12px' }}
        ...
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          <button onClick={() => onSkip(file)}
            style={{ padding: '8px 16px', border: '1px solid transparent', borderRadius: '8px', color: '#888', fontSize: '12px', fontWeight: 600, cursor: 'pointer', background: 'transparent' }}>
            Use Full Image
          </button>
          <button onClick={handleCrop} disabled={!canCrop}
            style={{ padding: '8px 18px', border: 'none', borderRadius: '8px', color: '#1a1a1a', fontSize: '12px', fontWeight: 700, cursor: canCrop ? 'pointer' : 'not-allowed', background: canCrop ? '#E88FAC' : 'transparent', opacity: canCrop ? 1 : 0.5 }}>
            Crop & Add
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ background: 'var(--card-bg-solid)', border: '1px solid transparent', borderRadius: '16px', padding: '28px', width: '500px', maxWidth: '95vw', maxHeight: '85vh', overflowY: 'auto' }}
        ...
        <div style={{ marginBottom: '14px' }}>
          ...
          <textarea
            ...
            style={{
              ...
              border: '1px solid transparent', borderRadius: '8px', color: '#4a4a4a',
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <button
              ...
              style={{ padding: '7px 14px', fontSize: '12px', fontWeight: 600, background: 'rgba(232, 160, 160, 0.04)', color: '#888', border: '1px solid transparent', borderRadius: '6px', cursor: 'pointer', opacity: (uploading || cropQueue.length > 0) ? 0.6 : 1 }}>
        ...
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          <button onClick={onClose} disabled={uploading}
            style={{ padding: '9px 18px', border: 'none', borderRadius: '8px', color: '#1a1a1a', fontSize: '13px', fontWeight: 600, cursor: 'pointer', background: 'transparent' }}>
            Cancel
          </button>
          <button onClick={handleSubmit} disabled={uploading || !feedback.trim()}
            style={{
              ...
              background: uploading || !feedback.trim() ? 'transparent' : '#ef4444', opacity: uploading ? 0.6 : 1,
            }}>
    ...
    <div onClick={e => e.target === e.currentTarget && onClose()}
      ...
      <div style={{ position: 'relative', maxHeight: '90vh', maxWidth: '420px', width: '100%' }}>
        ...
        <button onClick={onClose}
          style={{ position: 'absolute', top: '-14px', right: '-14px', background: 'rgba(232, 160, 160, 0.04)', border: '1px solid transparent', borderRadius: '50%', width: '32px', height: '32px', color: '#1a1a1a', fontSize: '16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ×
        </button>
      </div>
    </div>
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
        ...
        <button onClick={fetchTasks}
          style={{ padding: '6px 14px', fontSize: '12px', fontWeight: 600, background: 'var(--card-bg-solid)', color: '#888', border: '1px solid transparent', borderRadius: '6px', cursor: 'pointer' }}>
          Refresh
        </button>
      </div>
          ...
          <div className="admin-editor-notif" style={{ flexShrink: 0 }}>
            {notifStatus === 'idle' && (
              <button onClick={handleEnableNotifications}
                style={{ fontSize: '11px', fontWeight: 600, padding: '5px 12px', borderRadius: '8px', border: '1px solid transparent', background: 'transparent', color: '#999', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                🔔 Notifications
              </button>
            )}
