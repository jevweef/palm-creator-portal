...
export default function ContentMovement() {
  ...
  const th = { textAlign: 'right', padding: '11px 13px', fontSize: 11, letterSpacing: '0.04em', textTransform: 'uppercase', color: MUTED, fontWeight: 600, whiteSpace: 'nowrap' }
  const td = { textAlign: 'right', padding: '12px 13px', fontSize: 14, color: FG, borderTop: `1px solid ${BORDER}`, whiteSpace: 'nowrap' }
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1120 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 18 }}>
        ...
        <button onClick={load} disabled={loading}
          style={{ flex: 'none', background: 'transparent', border: `1px solid ${BORDER}`, color: MUTED2, borderRadius: 8, padding: '5px 13px', fontSize: 12, cursor: loading ? 'default' : 'pointer' }}>
                  ...
                  {data.rows.map(r => {
                    ...
                    return (
                      <tr key={r.id} style={{ opacity: busy ? 1 : 0.4 }}>
                        ...
                        <td style={{ ...td, fontWeight: 600, color: r.runway != null && r.runway < 1.5 ? PACE.low : FG }}>{r.runway != null ? r.runway + 'w' : '—'}</td>
                      </tr>
                    )
                  })}
                </tbody>
