...
/**
 * Mirror an Inspiration record's DB Share Link video to Cloudflare Stream.
 * One video per inspo; one Stream UID stored back on the record.
 *
 * @param {object} record - Airtable Inspiration record. Must have:
 *   id, fields: { DB Share Link?, DB Raw = 1?, Stream UID? }
 * @returns {Promise<{ ok, uid?: string, alreadyExisted?: boolean, skipped?: boolean, reason?: string, error?: string }>}
 */
export async function mirrorInspirationToCloudflareStream(record) {
  if (!isCloudflareStreamConfigured()) {
    return { ok: false, error: 'CF Stream not configured' }
  }
  const f = record?.fields || {}
  if (f['Stream UID']) {
    return { ok: true, skipped: true, reason: 'already mirrored', uid: f['Stream UID'] }
  }
  const link = f['DB Share Link'] || f['DB Raw = 1']
  if (!link) {
  ...
  let uid
  try {
    const result = await uploadVideoByUrl(rawDropboxUrl(link), {
      airtableId: record.id, table: 'Inspiration',
    })
    uid = result.uid
  } catch (err) {
  ...
  const res = await fetch(
    `https://api.airtable.com/v0/${OPS_BASE}/${INSPIRATION_TABLE}/${record.id}`,
    {
      method: 'PATCH',
      headers: {
        Authorization: `Bearer ${process.env.AIRTABLE_PAT}`,
  ...
  return { ok: true, uid, alreadyExisted: false }
}
