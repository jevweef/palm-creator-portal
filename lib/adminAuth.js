...
/**
 * Live Chat AI console access — the interactive whale/AI-chatting tool.
 *
 * Admins are always in. A chat_manager needs the per-user `liveChatAccess`
 * flag flipped ON from /admin/team. The flag is read via currentUser(), which
 * hits Clerk's Backend API (NOT the cached session token), so an admin toggling
 * it OFF cuts her access on the very next request — an instant kill-switch.
 *
 * This is a DELIBERATE, per-person, revocable exception to the chat-manager
 * no-AI default (the feedback_chat_manager_no_ai invariant still governs the
 * /photo-library view). Only flagged managers ever see AI here.
 */
export async function requireLiveChatAccess() {
  const { userId } = auth()
  if (!userId) {
    throw new Response(JSON.stringify({ error: 'Unauthorized' }), {
      status: 401,
  ...
  if (role === 'chat_manager' && user?.publicMetadata?.liveChatAccess === true) return user

  throw new Response(JSON.stringify({ error: 'Forbidden — Live Chat access not enabled' }), {
    status: 403,
    headers: { 'Content-Type': 'application/json' },
  })
}
...
// Look up a set of records by record ID, chunked so the OR(RECORD_ID()=...)
// formula never grows the request URL past Airtable's ~16KB nginx limit.
// Without chunking, a large active queue (~200+ linked Assets) sends a URL
// long enough to be rejected with `414 Request-URI Too Large`.
export async function fetchAirtableRecordsByIds(table, ids, options = {}) {
