...
/**
 * Close a file request (disables new uploads, keeps the record).
 * Required step before delete.
 */
export async function closeDropboxFileRequest(accessToken, rootNamespaceId, id) {
  const res = await fetch('https://api.dropboxapi.com/2/file_requests/update', {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({ id, open: false }),
  })
  if (!res.ok) {
    const err = await res.text()
    throw new Error(`Dropbox file_requests/update failed: ${res.status} ${err}`)
  }
  return res.json()
}

/**
 * Delete file requests by ID. They must be closed first.
 */
export async function deleteDropboxFileRequests(accessToken, rootNamespaceId, ids) {
  if (!ids.length) return
  const res = await fetch('https://api.dropboxapi.com/2/file_requests/delete', {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
  ...
  if (!res.ok) {
    const err = await res.text()
    throw new Error(`Dropbox file_requests/delete failed: ${res.status} ${err}`)
  }
  return res.json()
}
