...
// ffmpeg-static is a 50MB binary — only require() it the first time a
// video helper actually runs, so cold starts on photo-only routes stay fast.
...
/**
 * Upload raw image bytes (e.g. a JPEG buffer extracted by ffmpeg) to
 * Cloudflare Images. Same idempotency model as uploadImageByUrl — pass
 * the Airtable record ID as `idHint` and re-runs no-op via 5409.
 *
 * @param {Buffer|Uint8Array} bytes - Image bytes to upload.
 * @param {string} [idHint] - Optional custom CF image ID.
 * @param {string} [contentType] - Defaults to 'image/jpeg'.
 * @returns {Promise<{id: string, alreadyExisted: boolean, raw: object}>}
 */
export async function uploadImageBytes(bytes, idHint = null, contentType = 'image/jpeg') {
  assertConfigured()
  if (!bytes || !bytes.length) throw new Error('uploadImageBytes: bytes required')

  const form = new FormData()
  // CF accepts the raw blob — Node 18+ has Blob built in.
  const blob = new Blob([bytes], { type: contentType })
  const ext = contentType === 'image/png' ? 'png' : 'jpg'
  form.append('file', blob, `frame.${ext}`)
  if (idHint) form.append('id', idHint)
  form.append('requireSignedURLs', 'false')
...
/**
 * Run ffmpeg with the given args and return the produced bytes (or throw).
 */
function runFfmpeg(args, outputPath) {
  return new Promise((resolve) => {
    execFile(getFfmpegPath(), args, { timeout: 25000 }, async (err, _stdout, stderr) => {
...
/**
 * Download a video file from a Dropbox shared URL and extract a single frame
 * as a JPEG buffer. Tries seeking to `atSeconds`, then the first frame as a
 * fallback for very short or quirky files.
 *
 * @param {string} videoUrl - Dropbox share link (any form — we'll force ?raw=1).
 * @param {number} [atSeconds] - Where to grab the frame (default 1s).
 * @returns {Promise<Buffer>} JPEG bytes
 */
export async function extractVideoFrameJpeg(videoUrl, atSeconds = 1) {
  if (!videoUrl) throw new Error('extractVideoFrameJpeg: videoUrl required')

  const cleanLink = videoUrl
    .replace(/[?&]dl=0/, '')
    .replace(/[?&]raw=1/, '')
    .replace(/[?&]dl=1/, '')
  const rawUrl = cleanLink + (cleanLink.includes('?') ? '&raw=1' : '?raw=1')

  const id = `${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
  const inputPath = join(tmpdir(), `cfvf_${id}.mp4`)
  const outputPath = join(tmpdir(), `cfvf_${id}.jpg`)
  ...
  try {
    ...
    if (ct.includes('text/html') || buf.slice(0, 100).toString('utf8').includes('<!DOCTYPE html')) {
      throw new Error('Dropbox returned HTML — share link probably not "anyone with link"')
    }
    await writeFile(inputPath, buf)

    const ts = Math.max(0, Math.min(Number(atSeconds) || 0, 9999))
    const baseFilters = '-vf format=yuv420p -pix_fmt yuvj420p -q:v 3'.split(' ')
    const strategies = [
      ['-y', '-i', inputPath, '-ss', String(ts), '-frames:v', '1', '-update', '1', ...baseFilters, outputPath],
      ['-y', '-ss', String(ts), '-i', inputPath, '-frames:v', '1', '-update', '1', ...baseFilters, outputPath],
      ['-y', '-i', inputPath, '-frames:v', '1', '-update', '1', ...baseFilters, outputPath], // first frame
    ]
...
/**
 * Extract a frame from a video Asset and store it on the Asset as a CDN URL.
 *
 * Used when a video has no Airtable Thumbnail attachment to mirror — the only
 * way to get a poster onto CF Images is to pull the video bytes ourselves,
 * ffmpeg out a frame, and upload it. Idempotent via the asset's record ID
 * as the CF custom ID.
 *
 * Prefers `Edited File Link` (the editor's final cut) over the raw
 * `Dropbox Shared Link`, so done tasks display a frame from what was
 * actually delivered.
 *
 * @param {object} asset - Airtable Assets record.
 * @returns {Promise<{ ok: boolean, skipped?: boolean, reason?: string,
 *   error?: string, cdnUrl?: string, imageId?: string }>}
 */
export async function mirrorVideoFrameToCloudflare(asset) {
  ...
  if (!link) {
    return { ok: false, skipped: true, reason: 'no video link' }
  }

  let frameBytes
  try {
    frameBytes = await extractVideoFrameJpeg(link, 1)
  } catch (err) {
    return { ok: false, error: `ffmpeg: ${err.message}` }
  }

  let imageId
  let alreadyExisted = false
  try {
    const result = await uploadImageBytes(frameBytes, asset.id)
    imageId = result.id
