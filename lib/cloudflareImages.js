...
export async function mirrorAssetToCloudflare(asset) {
  ...
  // Image-type check — same logic as the chat-wall API
  ...
  if (!isImage) {
    return { ok: false, skipped: true, reason: 'not an image asset' }
  }
  ...
  const cleanLink = link
    ...
    .replace(/[?&]raw=1/, '')
    .replace(/[?&]dl=1/, '')
  const sourceUrl = cleanLink + (cleanLink.includes('?') ? '&raw=1' : '?raw=1')
