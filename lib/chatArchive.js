...
/** Oldest message across UNMERGED chunk shards — lets a resumed pull continue
 *  from where an interrupted round stopped (tab closed mid-pull) instead of
 *  re-buying pages that already sit in shards awaiting finalize. */
export async function oldestShardTip(creatorName, fanName, fanUsername) {
  try {
    const token = await getDropboxAccessToken()
    const rootNs = await getDropboxRootNamespaceId(token)
    const base = chatBasePath(creatorName, fanName, fanUsername)
    ...
    for (const e of entries || []) {
      const buf = await downloadFromDropbox(token, rootNs, `${base}/chunks/${e.name}`)
      if (!buf) continue
      for (const m of JSON.parse(buf.toString('utf8')) || []) {
        if (m?.id != null && m?.createdAt && (!tip || m.createdAt < tip.createdAt)) tip = { id: m.id, createdAt: m.createdAt }
      }
    }
    return tip
  } catch { return null } // no chunks folder — nothing pending
}
