 ...
 */
export async function fetchChatHistory(accountId, fanId, { sinceDate = null, maxPages = 40, startCursor = null } = {}) {
  const all = []
  const seenIds = new Set()
  let cursor = startCursor ? String(startCursor) : null
  let pages = 0
  let credits = 0
  let complete = false // true when we reached the thread's real start/window
  ...
  for (let i = 0; i < maxPages; i++) {
    ...
    // Past the end of the thread the API keeps answering (repeating data)
    // instead of returning empty — every wasted round still bills a credit
    // (334-message chat burned the full 40-page cap). Stop as soon as a page
    // brings nothing new.
    const fresh = batch.filter((m) => m?.id != null && !seenIds.has(String(m.id)))
    if (!fresh.length) { complete = true; break }
    for (const m of batch) if (m?.id != null) seenIds.add(String(m.id))
    all.push(...batch)
    pages++
    ...
    // NOTE: page sizes vary wildly (19–100 per page regardless of limit) —
    // a short page is NOT the end of the thread. The no-new-ids break above
    ...
    if (!next || String(next) === String(cursor)) { complete = true; break }
    cursor = next
    await sleep(150)
  }
...
function fmtDate(iso) {
  // Year included — archives span years now, and "Jun 19 → Jul 3" hid a
  // 12-month range (looked like two weeks).
  return new Date(iso).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: ET })
}
...
export async function createDataExport({ type, accountIds, startDate, endDate, fileType = 'csv', options = null, autoStart = true }) {
  const json = await ofApi('/data-exports', {
    method: 'POST',
    body: { type, account_ids: accountIds, start_date: startDate, end_date: endDate, file_type: fileType, auto_start: autoStart, ...(options ? { options } : {}) },
  })
...
export async function cancelDataExport(exportId) {
  const json = await ofApi(`/data-exports/${exportId}/cancel`, { method: 'POST' }).catch(() => null)
  return json?.data ?? json
}

/** Wait for the credit-cost calculation on an unstarted export (fast). */
export async function waitForExportEstimate(exportId, { intervalMs = 4000, maxWaitMs = 90000 } = {}) {
  const started = Date.now()
  while (Date.now() - started < maxWaitMs) {
    const d = await getDataExport(exportId)
    if (d.credit_cost != null || d.status === 'awaiting_start' || d.status === 'calculated') return d
