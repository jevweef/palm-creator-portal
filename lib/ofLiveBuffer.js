...
export async function readLiveMerged(accountId) {
  ...
  // Read-mostly: only COMPACT (rewrite the buffer + clean up) when pending
  // has built up — every reader rewriting on every poll made concurrent
  // readers overwrite each other and events visibly blinked out of the
  // Stream (2026-07-04). Small pending sets merge in memory only.
  const shouldCompact = names.length > 25
  if (names.length && !shouldCompact) {
