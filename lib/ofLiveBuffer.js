...
function parseRows(rows, cap = 400) {
  const out = []
  const seen = new Set()
  for (const r of rows || []) {
    try {
      const e = JSON.parse(r[0])
      // Sale events written before 2026-07-06 carry zoneless UTC stamps
      // ("YYYY-MM-DD HH:MM:SS"). Normalize to ISO at read time — space
      // sorts below 'T', so mixed formats made the newest-400 cut drop
      // old sales below same-day messages (Sales tab starved to a handful).
      if (e?.at && !String(e.at).includes('T')) {
        e.at = String(e.at).replace(' ', 'T') + (String(e.at).includes('+') ? '' : '+00:00')
      }
...
export async function readLiveMany(accountIds, { limit = 500 } = {}) {
  ...
  ;(res.data.valueRanges || []).forEach((vr, i) => {
    out[wanted[i]] = parseRows((vr.values || []).slice(-limit), limit)
  })
  return out
}
