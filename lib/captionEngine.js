...
- Funnel through DESIRE and intrigue, never solicitation. Stay 100% IG-reach-safe (no explicit, no platform names, no subscribe/pay). "check my bio" stays the only promo-adjacent line — and use it SPARINGLY (most captions should build the pull WITHOUT ever saying "bio"; repeating it post after post reads as spam).
...
6) Imperative micro-CTA — a light command. The ONLY promo-adjacent line allowed is "check my bio" and it must stay clean (no platform name, no subscribe/pay). Use this type RARELY — at most ONE of your options, and only when the clip really invites it. Most reels should NOT have a bio-mention caption.
...
Return your options via the submit_captions tool. Also set best_index to the ONE option that should auto-post: the one that best centers HER — her desirability, personality, or intrigue — and makes a viewer want more of her. NOT a prop/pet/joke, NOT the blandest-safest line, and do NOT default to a "check my bio" CTA — pick an in-voice, intriguing line over a bio plug unless the clip genuinely calls for the CTA (which is rare).`
...
const SUBMIT_CAPTIONS_TOOL = {
  ...
  parameters: {
    ...
    properties: {
      ...
      best_index: { type: 'integer', description: '0-based index into captions[] of the SINGLE best option to auto-post: the one that most centers HER (her desirability/personality/intrigue) and makes a viewer want more of her — NOT a prop/pet/joke, NOT the blandest-safest filler, and NOT a "check my bio" CTA (the funnel runs on intrigue, not on plugging the bio). Only pick a bio-CTA option when the clip genuinely invites it; otherwise prefer an in-voice intriguing line.' },
      captions: {
        type: 'array',
...
- Pass Instagram's published RECOMMENDATIONS GUIDELINES — the separate, stricter bar IG uses to decide whether a Reel is eligible to be shown to NON-followers (Explore / Reels / hashtags). These guidelines are NOT the same as what's allowed to stay up: IG explicitly will NOT recommend content that is "sexually explicit or suggestive, such as pictures of people in see-through clothing," near-nudity, or implied/implicitly sexual imagery — such content stays visible to her followers but is demoted and never featured to the broader audience. If a frame could trip that filter — i.e. it might make the reel NOT featureable to all audiences — it is TOO EXPLICIT for a thumbnail. Flag it; do not use it.
  REJECT (even fully clothed, no nudity): cleavage as the focal point, emphasized / pushed-up / centered breasts, under-boob or side-boob, butt or crotch emphasis or close-ups, a sexualized pose (arched back, spread legs, bent over, on a bed), lingerie/bikini framed to sexualize, see-through or wet clothing showing detail, a suggestive mouth (licking / biting / sucking), or hands or camera drawing the eye to chest or crotch.
  ALLOW: a face-forward or full-figure shot where she looks great but the FRAMING isn't sexualized — the kind of frame a mainstream brand could put on Explore. Confident and attractive is fine; sexually suggestive is not.
...
If EVERY otherwise-good frame of her would fail that Recommendations-Guidelines bar — i.e. there is no flattering frame that isn't sexually suggestive enough to make the reel un-featureable to all audiences — set too_risque=true and leave the timestamps null. We then leave the thumbnail blank ON PURPOSE so the grid fills it from the creator's hand-picked thumbnail queue instead. When it's a close call, lean too_risque=true: a slightly-too-safe thumbnail from her queue is always better than a suggestive frame that quietly tanks the reel's reach.
...
async function runWithFallback(requestBody, toolName = 'submit_captions') {
  ...
  // Prefer the configured model, but FALL BACK to a secondary when the primary
  // stays overloaded ("high demand" / 503 / 429) after retries — a slightly
  // weaker caption beats a failed reel that strands the pipeline (Gemini Flash
  // gets throttled in bursts). Each model gets its own short backoff ladder.
  const FALLBACK_MODEL = process.env.GEMINI_CAPTION_FALLBACK || 'gemini-2.5-flash'
  const models = FALLBACK_MODEL && FALLBACK_MODEL !== GEMINI_MODEL
  ...
  for (const model of models) {
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        ...
        lastErr = e
        if (e.modelNotFound) break          // bad id for this model → try the next model
        if (!e.transient) throw e           // real error (e.g. SAFETY block) → surface now
        if (attempt < 3) await sleep(700 * (attempt + 1))  // 0.7s, 1.4s, 2.1s backoff
      }
    }
    // primary still overloaded after its retries → fall through to the next model
  }
  throw lastErr
}
