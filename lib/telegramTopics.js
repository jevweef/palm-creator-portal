...
/**
 * Create a forum topic in the SMM group with an arbitrary name. Returns the
 * message_thread_id, or null if the group isn't configured (caller skips).
 */
export async function createSmmTopic(name) {
  ...
  const clean = (name || '').trim().slice(0, 64) // Telegram 64-char limit
  if (!clean) throw new Error('topic name is required')
  const result = await tg('createForumTopic', { chat_id: SMM_GROUP_CHAT_ID, name: clean })
  return result.message_thread_id
}
