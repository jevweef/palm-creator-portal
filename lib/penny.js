...
export async function assignChannels(creatorId, creatorFields) {
  ...
  // Real reels → IG/FB round-robin, balanced against this creator's running
  // IG/FB history. CRITICAL: count over ALL of the creator's channeled reels in
  // the window — including already-sent/posted ones — NOT just the unsent
  // in-flight pool (creatorPosts). The hourly push sends staged posts within
  // minutes, so counting only unsent posts meant the tally reset to ~0 every
  // cycle and ties always broke to IG → per-creator splits drifted badly
  // (seen live: 48/21, 27/7, 25/59). allRecent already spans the last 60 days
  // (sent posts keep their Scheduled Date), which is ample balancing memory.
  if (realQueue.length && hasIgFb) {
    const count = { IG: 0, FB: 0 }
    for (const p of allRecent) {
      if (!linkedIds(p.fields?.Creator).includes(creatorId)) continue
