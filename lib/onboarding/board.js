...
export const BOARD_TILES = [
  ...
  },
  {
    ...
    // sends — and until now nothing created them (the portal's most silent
    // per-creator blocker). One click creates the missing topics in the SMM
    // master group and writes the ids to ops Palm Creators.
    key: 'tg-topics', group: 'routing', label: 'Delivery topics (IG / FB / AI)', dependsOn: [],
    instructions: 'Post Prep and Grid Planner can’t deliver ANYTHING for this creator until these Telegram topics exist. One click creates the missing ones in the SMM group and wires them up. AI topic only applies when AI content is enabled.',
    status: ({ ops }) => {
