...
export const BOARD_TILES = [
  ...
  },
  {
    ...
    instructions: 'The creator fills out the onboarding questionnaire. "Done" = they hit Submit; partial answers still show below. Required before go-live and feeds the DNA profile.',
    status: ({ phase1 }) => stepDone(phase1, 'survey'),
    detail: ({ phase1, surveyAnswerCount = 0 }) => {
      const done = stepDone(phase1, 'survey') === 'done'
      if (done) return surveyAnswerCount ? `Submitted · ${surveyAnswerCount} answers` : 'Submitted'
      if (surveyAnswerCount > 0) return `${surveyAnswerCount} answers saved · NOT submitted`
      return 'Not started'
    },
  ...
  {
    key: 'survey-chat-team', group: 'inputs', label: 'Survey → chat team', dependsOn: ['chat-team'],
    instructions: 'Preview the creator’s survey (PDF + CSV) — if it looks good, Send to the Palm Chatting group. Empty answers are dropped. N/A until there are answers.',
    status: ({ of, surveyAnswerCount = 0 }) => (isTrue(of['Survey Sent to Chat Team']) ? 'done' : (surveyAnswerCount > 0 ? 'todo' : 'na')),
    detail: ({ of, surveyAnswerCount = 0 }) => (isTrue(of['Survey Sent to Chat Team'])
      ? (of['Survey Sent to Chat Team At'] ? `Sent ${String(of['Survey Sent to Chat Team At']).slice(0, 10)}` : 'Sent')
      : (surveyAnswerCount > 0 ? `${surveyAnswerCount} answers ready` : 'No answers yet')),
    action: () => ({ type: 'survey-send' }),
  },
  {
  ...
  },
  {
    ...
    instructions: 'Decide whether Palm runs this creator’s social media. Toggle “Social Media Editing” on their profile. If we’re NOT doing their socials, the IG / quota / AI steps don’t apply.',
    status: ({ ops }) => (isTrue(ops['Social Media Editing']) ? 'done' : 'todo'),
    detail: ({ ops }) => (isTrue(ops['Social Media Editing']) ? 'Yes — Palm runs socials' : 'Not set — decide first'),
