...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  body {
    ...
    background: #fdf5f0; color: #000; width: 800px;
    position: relative;
  }
  .page {
    padding: 45px 60px 35px; position: relative;
...
<div class="page">
  ...
  <div class="section">
    ...
    <div class="section-body">
      ...
      <ul>
        ...
        <li>DMCA leak prevention and takedown team</li>
  ...
  <div class="signature-block">
    <div class="sig-col">
      ...
      <div class="sig-field">${signedName || ''}</div>
      <div class="sig-field-label">Signature</div>
      <div class="sig-field">${signatureDataUrl ? `<img src="${signatureDataUrl}" class="sig-image" />` : ''}</div>
      <div class="sig-field-label">Date</div>
      <div class="sig-field">${signedDate ? fmtDate(signedDate) : ''}</div>
    </div>
    <div class="sig-col">
      <div class="sig-label">Agency</div>
      <div class="sig-field-label">Name</div>
      <div class="sig-field">Palm Digital Management LLC</div>
      <div class="sig-field-label">Signature</div>
...
export async function generateContractPdf(contract) {
  ...
  const browser = await puppeteer.launch({
    args: chromium.args,
    defaultViewport: { width: 800, height: 2400, deviceScaleFactor: 2 },
  ...
  // Scale to fit letter-width pages, potentially multiple pages
  const pageWidth = 400
  const imageAspect = bodyHeight / 800
  const totalHeight = pageWidth * imageAspect
  const pageHeight = 565 // same as invoice

  // Single page if it fits, otherwise scale down
  const pdfPage = pdfDoc.addPage([pageWidth, Math.max(totalHeight, pageHeight)])
  pdfPage.drawImage(jpegImage, {
    x: 0, y: pdfPage.getHeight() - totalHeight,
    width: pageWidth,
    height: totalHeight,
  })
