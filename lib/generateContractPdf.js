...
function buildContractHtml(contract) {
  const {
    ...
    agencyName,
    agencySignDate,
    amendments = [], // [{title, text}] — accepted per-creator amendments; supersede conflicting boilerplate
  } = contract
  ...
  const amendmentsSection = Array.isArray(amendments) && amendments.length ? `
  <div class="section">
    <div class="section-title">9. Amendments</div>
    <div class="section-body">
      The parties agree to the following amendments, which supersede any conflicting terms elsewhere in this Agreement:
      <ul>
        ${amendments.map((a) => `<li><strong>${esc(a.title)}:</strong> ${esc(a.text)}</li>`).join('\n        ')}
      </ul>
    </div>
  </div>
` : ''
  const ackNumber = amendmentsSection ? '10' : '9'
...
<div class="page">
  ...
  <div class="section">
    ...
    <div class="section-body">
      The Agency and the Creator agree that the Agency's compensation will be a percentage of the revenue generated by the Creator on OnlyFans and/or Fansly. The applicable share is as follows in USD:
      ${(() => {
        // commissionTier can be: array of tiers [{pct, upTo}], old format {tierPct, threshold}, or null
        let tiers = null
        if (Array.isArray(commissionTier)) {
          tiers = commissionTier
        } else if (commissionTier && commissionTier.tierPct) {
          // Legacy format — convert
          tiers = [
            { pct: Math.round(commissionPct * 100), upTo: commissionTier.threshold },
            { pct: commissionTier.tierPct, upTo: null },
          ]
        }
        if (tiers && tiers.length > 1) {
          return tiers.map((t, i) => {
            ...
            if (i === 0 && t.upTo) {
              return `<div class="highlight">${pct} of the page up to $${Number(t.upTo).toLocaleString()}/month</div>`
            } else if (i > 0) {
              const prev = tiers[i - 1]
              return `<div class="highlight">${pct} of the page above $${Number(prev.upTo).toLocaleString()}/month</div>`
            }
            return `<div class="highlight">${pct} of the page PER MONTH</div>`
          }).join('\n      ')
        }
        return `<div class="highlight">${pctDisplay} of the page PER MONTH</div>`
      })()}
      These percentages are not expected but subject to change due to unforeseen factors such as but not limited to travel, social media account recovery services, or special exceptions. Any changes to the revenue share will be communicated in writing by the Agency prior to any business being done that should require, and the Creator agrees to adhere to these updated terms. Any of the above can not be changed unless there is explicit written agreement by the creator prior.
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="section-body">
      <ul>
        <li><strong>Governing Law/Venue:</strong> This Agreement shall be governed by and construed in accordance with the laws of the State of Florida. Any action to enforce, or for breach of, this Agreement shall be brought exclusively in the state or federal courts located in Palm Beach County, Florida.</li>
...
${amendmentsSection}
  <div class="section">
    <div class="section-title">${ackNumber}. Acknowledgement</div>
    <div class="section-body">
...
/**
 * Render the agreement with per-creator amendments applied IN PLACE.
 *
 * `contract.amendments` items come in two shapes:
 *   { title, find, replace } — edit the original clause: `find` is an exact,
 *     unique substring of the base HTML; `replace` is the new clause HTML.
 *     The section she complained about actually changes — nothing bolted on.
 *   { title, text }          — no in-place target (or a find that no longer
 *     matches after template/values drift): rendered in the numbered
 *     "Amendments" section at the end so nothing is ever silently dropped.
 *
 * `highlight: true` (admin previews only) wraps applied replacements in a
 * yellow marker so the old-vs-new compare shows exactly what changed.
 */
// The hand-editable body region: every numbered section, excluding the header
// (logo/title/parties) and the signature block — those stay templated so
// signing/dates keep interpolating correctly.
export function contractBodyBounds(html) {
...
export function renderContract(contract, { highlight = false } = {}) {
  // Admin hand-edited body wins outright — the AI amendments were already baked
  // into the text when it was captured for editing, so they're skipped here.
  const override = typeof contract.bodyOverride === 'string' ? contract.bodyOverride.trim() : ''
  if (override) {
    const base = buildContractHtml({ ...contract, amendments: [] })
    const b = contractBodyBounds(base)
    if (b) return base.slice(0, b.start) + override + '\n' + base.slice(b.end)
  }

  const items = Array.isArray(contract.amendments) ? contract.amendments : []
  const inPlace = items.filter((i) => i && i.find && i.replace)
  const textual = items
    .filter((i) => i && i.text && !(i.find && i.replace))
    .map((i) => ({ title: i.title || 'Amendment', text: i.text }))

  // Pass 1 — probe which edits still match the base document (unique hit).
  const base = buildContractHtml({ ...contract, amendments: [] })
  const applied = []
  const fallbacks = [...textual]
  for (const e of inPlace) {
    const idx = base.indexOf(e.find)
    const unique = idx !== -1 && base.indexOf(e.find, idx + e.find.length) === -1
    if (unique) applied.push(e)
    else fallbacks.push({ title: e.title || 'Amendment', text: String(e.replace).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() })
  }

  // Preview marker. A <span> can't legally wrap block elements (<li>, <div>) —
  // browsers hoist it and the yellow silently vanishes for whole-clause edits.
  // So: tag-led replacements get the background injected into their opening
  // tag(s); plain-text replacements get the span wrap.
  const mark = (rep) => {
    const s = String(rep)
    const m = s.match(/^\s*<([a-zA-Z0-9]+)/)
    if (m) {
      const tag = m[1]
      return s.replace(new RegExp(`<${tag}(?![a-zA-Z0-9])`, 'g'), `<${tag} style="background:#fff3bd;border-radius:2px;"`)
    }
    return `<span style="background:#fff3bd;border-radius:2px;">${s}</span>`
  }

  // Pass 2 — real build (with any fallback section), then splice the edits in.
  let html = buildContractHtml({ ...contract, amendments: fallbacks })
  for (const e of applied) {
    html = html.replace(e.find, highlight ? mark(e.replace) : e.replace)
  }
  return html
}
