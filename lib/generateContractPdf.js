...
export async function generateContractPdf(contract) {
  ...
  // Scale image to fit letter-width, then paginate
  const pageWidth = 612   // US Letter width in points
  const pageHeight = 792  // US Letter height in points
  const scale = pageWidth / 800
  const totalHeight = bodyHeight * scale
  const numPages = Math.ceil(totalHeight / pageHeight)

  for (let i = 0; i < numPages; i++) {
    const pdfPage = pdfDoc.addPage([pageWidth, pageHeight])
    // Position the image so the correct slice is visible in this page's viewport
    const yOffset = pageHeight - totalHeight + (i * pageHeight)
