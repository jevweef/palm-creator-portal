...
function formatMessage({ event, creator, projectName, notes, fileCount, revisionCount, assignedEditor }) {
  const safeCreator = creator || 'Unknown creator'
  const safeProject = projectName || 'Untitled project'
  const portalLink = `${PORTAL_BASE}/editor?tab=oftv`
  const adminLink = `${PORTAL_BASE}/admin/editor?tab=oftv`
