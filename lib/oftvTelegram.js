...
function formatMessage({ event, creator, projectName, projectId, notes, fileCount, revisionCount, assignedEditor }) {
  ...
  switch (event) {
    ...
    // Editor revised a cut after creator feedback. Skips the admin gate —
    // creator's revisions go directly back to the creator. Admin sees this
    // as awareness only ("here's what's happening") rather than a review
    // request.
    case 'revised_cut_to_creator':
      return [
        ...
        assignedEditor ? `Editor: ${assignedEditor}` : null,
        ``,
        `Editor addressed ${safeCreator}'s feedback. Sent directly back to her — no admin gate.`,
      ].filter(Boolean).join('\n')
