...
// MINIMUM bitrate reduction needed. For files barely over this is an ~8%
// bitrate shave — visually indistinguishable from source.
export async function compressVideo(inputBuffer, inputName, { targetMB = 47, aggressive = false } = {}) {
  const id = Date.now()
...
export async function makeTelegramReady(inputBuffer, inputName) {
  ...
  // Already MP4 + under 50MB → ship as-is. Most common case for editor
  // uploads that are already capped at 1080×1920 H.264.
  if (isMp4 && sizeBytes <= TELEGRAM_MAX_BYTES) {
    return { buffer: buf, filename: inputName, mime: 'video/mp4' }
  }

  // Non-MP4 but small enough → remux to MP4 (no quality loss).
  if (!isMp4 && sizeBytes <= TELEGRAM_MAX_BYTES) {
    ...
    // Remux output unexpectedly grew past 50MB — fall through to compress.
  }

  // Oversized → compress.
  const compressed = await compressVideo(buf, inputName, { targetMB: 47 })
