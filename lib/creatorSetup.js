...
// Canonical Dropbox template folder copied per-creator when set. When empty we
// fall back to building the folder tree by hand (current behavior). Set this to
// the path of Evan's master template folder (e.g. "/Palm Ops/_Templates/Creator")
// so each creator's tree is a 1:1 copy of what we maintain by hand. The template
// MUST contain the same relative subfolders buildFolderPaths() expects (Social
// Media/00_INCOMING_FILE_REQUEST and Long Form/10_UNREVIEWED_LIBRARY) so the
// file-request destinations line up.
const DROPBOX_CREATOR_TEMPLATE_PATH = (process.env.DROPBOX_CREATOR_TEMPLATE_PATH || '').replace(/\/+$/, '')
...
function buildFolderPaths(aka) {
  ...
  // Canonical tree — MUST match the full pipeline the established creators use
  // (Amelia/Caitie/Raya/Taby...). The stripped-down version created new creators
  // missing the downstream 30/40/50 stages, so editor moves later had nowhere to
  // land. Keep this in sync with the real creator folders.
  return {
    root,
    folders: [
      root,
      ...
      `${root}/Social Media/30_EDITED_EXPORTS`,
      `${root}/Social Media/35_FINALS_FOR_REVIEW`,
      `${root}/Social Media/40_READY_TO_POST`,
      `${root}/Social Media/50_POSTED_ARCHIVE`,
      `${root}/Long Form`,
      `${root}/Long Form/10_UNREVIEWED_LIBRARY`,
      `${root}/Long Form/20_NEEDS_EDIT`,
      `${root}/Long Form/30_EDITED_EXPORTS`,
      `${root}/Long Form/35_FINALS_FOR_REVIEW`,
      `${root}/Long Form/40_READY_TO_PUBLISH`,
      `${root}/Long Form/50_PUBLISHED_ARCHIVE`,
      `${root}/Long Form/60_THUMBNAILS_COVERS`,
    ],
 ...
 *
 * NOTE: we cannot match the linked Creator via filterByFormula. Airtable
 * evaluates `{Creator}` (a linked-record field) as the linked records' primary
 * *display values*, not their record IDs — so `FIND(recId, ARRAYJOIN({Creator}))`
 * never matches and would create a duplicate Onboarding record on every call.
 * Instead we narrow by the (denormalised) Creator Name, then match precisely in
 * JS against the Creator link array, which the REST API returns as record IDs.
 * If several already exist (legacy duplicates), we keep the earliest — the
 * original record carrying the real setup data.
 */
export async function getOrCreateOnboardingRecord(creatorRecordId, creatorName) {
  const linkMatches = (recs) =>
    recs
      .filter((r) => Array.isArray(r.fields['Creator']) && r.fields['Creator'].includes(creatorRecordId))
      .sort((a, b) => new Date(a.createdTime) - new Date(b.createdTime))

  // Fast path: narrow by Creator Name, then confirm the link by ID.
  if (creatorName) {
    const byName = await fetchHqRecords(HQ_ONBOARDING, {
      filterByFormula: `{Creator Name}="${String(creatorName).replace(/"/g, '\\"')}"`,
    })
    const matches = linkMatches(byName)
    if (matches.length > 0) return matches[0]
  }

  // Fallback: Creator Name may be blank/legacy — scan all and match by link.
  const all = await fetchHqRecords(HQ_ONBOARDING, {})
  const matches = linkMatches(all)
  if (matches.length > 0) return matches[0]
...
/**
 * Provision the creator's Dropbox folder tree. Idempotent.
 *
 * When DROPBOX_CREATOR_TEMPLATE_PATH is set, we copy the canonical template
 * folder to /Palm Ops/Creators/{aka} in a single copy_v2 (mirrors exactly what
 * an operator maintains by hand). If the destination already exists (re-run) the
 * copy is a no-op and we fall through to ensuring each expected subfolder exists.
 *
 * When the template path is NOT set, we build the tree by hand folder-by-folder
 * (the original behavior) — so this change is a no-op until the env var is set.
 */
export async function createCreatorFolders(accessToken, rootNamespaceId, aka) {
  ...
  if (DROPBOX_CREATOR_TEMPLATE_PATH) {
    const result = await copyDropboxFolder(accessToken, rootNamespaceId, DROPBOX_CREATOR_TEMPLATE_PATH, root)
    copiedFromTemplate = !result?.skipped
  }

  // Always ensure the expected subfolders exist. This is the full provisioning
  // path when no template is configured, and a cheap idempotent backstop after a
  // template copy (covers a re-run where the root already existed, or a template
  // that's missing one of the file-request destinations).
  for (const path of folders) {
    await createDropboxFolder(accessToken, rootNamespaceId, path)
  }

  return { rootPath: root, socialUploadPath, longformUploadPath, copiedFromTemplate }
}
...
export async function createDefaultSocialAccounts(creatorRecordId, aka, { igHandle } = {}) {
  ...
  // Dedup by exact Account Name. We CANNOT match the linked Creator via
  // filterByFormula — Airtable evaluates {Creator} as the linked record's
  // primary display value, not its ID, so FIND(recId, ARRAYJOIN({Creator}))
  // never matches and re-runs would create duplicate accounts. Account names
  // are deterministic ("{aka} - {suffix}"), so we match on those instead.
  const candidateNames = accountsToCreate.map(acc => `${aka} - ${acc.suffix}`)
  const nameFilter = `OR(${candidateNames.map(n => `{Account Name}="${n.replace(/"/g, '\\"')}"`).join(',')})`
  const existing = await fetchHqRecords(HQ_ACCOUNTS, {
    filterByFormula: nameFilter,
    fields: ['Account Name'],
  })
  const existingByName = new Map(existing.map(r => [r.fields['Account Name'], r.id]))

  const created = []
  for (const acc of accountsToCreate) {
    const name = `${aka} - ${acc.suffix}`
    if (existingByName.has(name)) {
      created.push({ name, recordId: existingByName.get(name), skipped: true })
      continue
    }
