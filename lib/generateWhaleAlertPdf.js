...
function buildWhaleAlertHtml({ creatorName, creatorAka, alert, analysis }) {
  ...
  // Monthly chart bars — ALL months from first purchase → now, no cap.
  // Empty months render as grey bars so the cool-off is visible; a long
  // silent tail is exactly what a dormant-whale reader needs to see.
  const monthlyHistory = a.monthlyHistory || []
  const accountNames = a.accountNames || []
  const multiAccount = accountNames.length > 1
  const chartMonths = monthlyHistory
  ...
  const dense = chartMonths.length > 14 // long spans: smaller labels, year markers
  const barsHtml = chartMonths.map((m, mi) => {
    ...
    return `
      <div style="display:flex;flex-direction:column;align-items:center;flex:1;">
        <div style="font-size:${dense ? 8 : 11}px;font-weight:600;color:${isLow ? '#DC2626' : 'rgba(0,0,0,0.55)'};min-height:${dense ? 11 : 16}px;margin-bottom:2px;">${m.spend > 0 ? fmtMoney(m.spend) : ''}</div>
        ...
        <div style="font-size:${dense ? 8 : 11}px;font-weight:500;color:rgba(0,0,0,0.35);margin-top:4px;white-space:nowrap;">${MO_NAMES[moNum] || ''}${(mi === 0 || moNum === 1) ? `'${m.month.slice(2, 4)}` : ''}</div>
      </div>`
  }).join('')
...
<body>
  ...
  <div style="display:flex;gap:10px;margin-bottom:20px;">
    ...
    <div style="flex:1;background:rgba(255,255,255,0.6);border:1px solid rgba(0,0,0,0.04);border-radius:10px;padding:12px;">
      ...
      <div style="font-size:20px;font-weight:800;color:${a.currentGap > a.medianGap * 3 ? '#DC2626' : '#EA580C'};margin-top:4px;">${a.currentGap}d ${(() => {
        // gapRatio isn't set by every alert path — derive it, and fall back
        // to the site's "vs ~Nd rhythm" phrasing rather than "undefinedx"
        const ratio = a.gapRatio || (a.medianGap > 0 && a.currentGap ? Math.round((a.currentGap / a.medianGap) * 10) / 10 : 0)
        if (ratio) return `<span style="font-size:12px;font-weight:500;color:rgba(0,0,0,0.3);">(${ratio}x his usual rhythm)</span>`
        if (a.medianGap) return `<span style="font-size:12px;font-weight:500;color:rgba(0,0,0,0.3);">(vs ~${a.medianGap}d rhythm)</span>`
        return ''
      })()}</div>
    </div>
    <div style="flex:1;background:rgba(255,255,255,0.6);border:1px solid rgba(0,0,0,0.04);border-radius:10px;padding:12px;">
