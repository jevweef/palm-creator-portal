...
function buildWhaleAlertHtml({ creatorName, alert, analysis }) {
  ...
  // Monthly chart bars — last 7 months
  const monthlyHistory = a.monthlyHistory || []
  const chartMonths = monthlyHistory.slice(-7)
  const maxMo = Math.max(...chartMonths.map(m => m.spend), 1)
  // Round up to nice number for chart
  ...
  const barsHtml = chartMonths.map(m => {
    const pct = Math.max((m.spend / chartMax) * 100, m.spend > 0 ? 3 : 0)
    const moNum = parseInt(m.month.slice(5))
    const isLow = m.spend < (a.monthlyAvg90 || 1) * 0.25
    ...
    return `
      <div style="display:flex;flex-direction:column;align-items:center;flex:1;gap:2px;">
        <div style="font-size:11px;font-weight:600;color:${isLow ? '#DC2626' : 'rgba(0,0,0,0.55)'};min-height:16px;">${m.spend > 0 ? fmtMoney(m.spend) : ''}</div>
        <div style="width:100%;max-width:55px;height:${pct}%;background:${barColor};border-radius:4px 4px 0 0;min-height:${m.spend > 0 ? 4 : 2}px;"></div>
        <div style="font-size:11px;font-weight:500;color:rgba(0,0,0,0.35);margin-top:3px;">${MO_NAMES[moNum] || ''}</div>
      </div>`
  }).join('')
  ...
  if (analysis?.managerBrief) {
    managerBriefHtml = `
      <div style="margin-top:20px;">
        <div style="font-size:10px;font-weight:700;color:rgba(0,0,0,0.25);text-transform:uppercase;letter-spacing:2.5px;margin-bottom:10px;">Chat Manager Brief</div>
        <div style="background:rgba(255,255,255,0.6);border:1px solid rgba(0,0,0,0.06);border-radius:12px;padding:16px 20px;font-size:13px;color:rgba(0,0,0,0.75);line-height:1.7;">
          ${formatAnalysisText(analysis.managerBrief)}
        </div>
      </div>`
  }
...
<body>
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;">
    <div>
      <div style="font-size:11px;font-weight:700;color:rgba(0,0,0,0.25);text-transform:uppercase;letter-spacing:2.5px;">Whale Alert</div>
      <div style="font-size:28px;font-weight:800;color:rgba(0,0,0,0.85);letter-spacing:-0.5px;margin-top:2px;">${creatorName}</div>
    </div>
  ...
  <div style="margin-bottom:20px;">
    <div style="font-size:24px;font-weight:800;color:rgba(0,0,0,0.85);letter-spacing:-0.5px;">${a.fan}</div>
    ${a.username ? `<div style="font-size:14px;font-weight:500;color:#E88FAC;margin-top:2px;">@${a.username}</div>` : ''}
  </div>
  ...
  <div style="display:flex;gap:10px;margin-bottom:20px;">
    ...
    <div style="flex:1;background:rgba(255,255,255,0.6);border:1px solid rgba(0,0,0,0.04);border-radius:10px;padding:12px;">
      <div style="font-size:10px;font-weight:600;color:rgba(0,0,0,0.25);text-transform:uppercase;letter-spacing:1px;">90d Avg/Mo</div>
      <div style="font-size:20px;font-weight:800;color:rgba(0,0,0,0.85);margin-top:4px;">${fmtMoney(a.monthlyAvg90)}</div>
    </div>
  </div>
