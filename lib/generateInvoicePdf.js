...
function nextFridayAfter(date) {
  const d = new Date(date)
  const daysAhead = (4 - d.getDay() + 7) % 7 || 7
...
function buildInvoiceHtml(invoice) {
  ...
  const payBy = nextFridayAfter(latestEnd)

  let serviceTitle, lineItems
  if (isMulti) {
    serviceTitle = 'Management Services'
    lineItems = periods.map((p, i) => {
      const periodLabel = p.label ? `${p.label} — ${fmtPeriod(p.start, p.end)}` : fmtPeriod(p.start, p.end)
      return `
    ...
    const p = periods[0]
    serviceTitle = `Management Services &mdash; ${fmtPeriod(p.start, p.end)}`
    lineItems = `
      <div class="line-item">
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .payment-row-label { font-size: 13px; font-weight: 500; color: rgba(0,0,0,0.3); min-width: 60px; }
  ...
  <div class="payment-section">
    <div class="payment-details">
      ...
      <div class="payment-info">
        ...
        <div class="payment-row"><span class="payment-row-label">Account</span><span class="payment-row-value">2900308524</span></div>
        <div class="payment-row"><span class="payment-row-label">Routing</span><span class="payment-row-value">267084131</span></div>
...
export async function generateInvoicePdf(invoice) {
  ...
  // Wrap JPEG in a minimal PDF (single image page)
  const jpegToPdf = buildJpegPdf(screenshotBuffer, 800, 1130)
  return Buffer.from(jpegToPdf)
}

function buildJpegPdf(jpegBytes, widthPx, heightPx) {
  // Convert pixels to points at 144 DPI (2x device scale factor → 72 DPI effective)
  const widthPt = widthPx / 2 * 72 / 72  // 800px at 2x = 400pt... actually just use 800/2 = 400
  const pageW = widthPx * 72 / 144  // 400pt
  const pageH = heightPx * 72 / 144 // 565pt
  ...
  const page = `3 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${pageW} ${pageH}] /Contents 5 0 R /Resources << /XObject << /Im0 4 0 R >> >> >>\nendobj\n`
  const imageStream = `4 0 obj\n<< /Type /XObject /Subtype /Image /Width ${widthPx * 2} /Height ${heightPx * 2} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${jpeg.length} >>\nstream\n`
  const imageEnd = '\nendstream\nendobj\n'
  ...
  xrefOffsets.push(pdf.length)
  // For binary data, we need to use Buffer
  const beforeImage = Buffer.from(pdf, 'ascii')
  const imageHeader = Buffer.from(imageStream, 'ascii')
  const imageFooter = Buffer.from(imageEnd, 'ascii')

  const contentOffset = beforeImage.length + imageHeader.length + jpeg.length + imageFooter.length
  xrefOffsets.push(contentOffset)
  const contentBuf = Buffer.from(content, 'ascii')

  // Build xref
  const xrefOffset = contentOffset + contentBuf.length
  let xref = `xref\n0 6\n0000000000 65535 f \n`
  ...
  return Buffer.concat([beforeImage, imageHeader, jpeg, imageFooter, contentBuf, xrefBuf])
}
