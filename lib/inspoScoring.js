 ...
 *
 * Reels explicitly tagged for the creator (Inspo Source → Palm Creators link,
 * surfaced as `forCreator`) get a flat +0.5 additive boost on top, so curated
 * source content clusters near the top of her feed.
 *
 ...
 */
/**
 * Strong additive boost applied to reels explicitly tagged for this creator
 * (via the Inspo Source → Palm Creators link, surfaced as `forCreator`).
 * Base component scores sum to ~[0,1], so a +0.5 bump clusters tagged reels
 * near the top while still letting her strongest organic matches interleave.
 */
const W_FORCREATOR = 0.5

/** True when this reel was explicitly tagged for the given creator. */
function isTaggedForCreator(r, creatorOpsId) {
  return !!creatorOpsId && Array.isArray(r.forCreator) && r.forCreator.includes(creatorOpsId)
}
...
export function scoreAndRankForYou(records, ctx = {}) {
  ...
  if (!creatorOpsId || Object.keys(creatorTagWeights).length === 0) {
    // No DNA → engagement-sorted fallback so callers always get a
    // deterministic ranked list to slice from. Tagged-for-this-creator reels
    // still float to the top so source→creator tagging works even before a
    // creator has any tag DNA.
    return [...records].sort((a, b) => {
      const ta = isTaggedForCreator(a, creatorOpsId)
      const tb = isTaggedForCreator(b, creatorOpsId)
  ...
  if (hideNiche) {
    pool = pool.filter((r) => r.effort !== 'Niche' || isTaggedForCreator(r, creatorOpsId))
  }
  ...
  const stamped = pool.map((r, i) => {
    ...
    const tagged = isTaggedForCreator(r, creatorOpsId)
    const forCreator = tagged ? W_FORCREATOR : 0
    const base = W_SEM * semantic + W_DNA * tag + W_VIR * viral + W_MGR * mgr + W_PER * per + forCreator
    const jit = applyJitter ? jitterForRecord(r.id) : 1
    return {
      ...r,
      forYouScore: base * jit,
      forYouComponents: { semantic, tag, viral, manager: mgr, personal: hasPersonal ? per : null, forCreator },
    }
  })
