...
export function getBucketsForRole(role) {
  if (role === 'admin') {
    // Admin gets one bucket per status — they want a kanban-like view
    // where every state has its own column. Final Submitted is still
    // styled urgent (red border + label prefix) since that's the only
    // bucket that strictly needs admin action.
    return [
      { key: 'final',   label: '🔴 Needs Your Review',  urgent: true,  statuses: [STATUSES.FINAL_SUBMITTED] },
      { key: 'awaiting', label: 'Awaiting Upload',       urgent: false, statuses: [STATUSES.AWAITING_UPLOAD] },
      { key: 'uploaded', label: 'Files Uploaded',        urgent: false, statuses: [STATUSES.FILES_UPLOADED] },
      { key: 'editing',  label: 'In Editing',            urgent: false, statuses: [STATUSES.IN_EDITING] },
      { key: 'arev',     label: 'Admin Revision',        urgent: false, statuses: [STATUSES.ADMIN_REVISION] },
      { key: 'sent',     label: 'Sent to Creator',       urgent: false, statuses: [STATUSES.SENT_TO_CREATOR] },
      { key: 'crev',     label: 'Creator Revision',      urgent: false, statuses: [STATUSES.CREATOR_REVISION] },
      { key: 'approved', label: 'Approved',              urgent: false, statuses: [STATUSES.APPROVED] },
