...
// Lazy singleton — created on first use, NOT at module load, so `next build`'s
// page-data collection doesn't crash when OPENAI_API_KEY is absent in the build
// environment (e.g. preview deploys). The OpenAI SDK throws in its constructor
// when there's no key, which would otherwise fail the whole build.
let _openai
