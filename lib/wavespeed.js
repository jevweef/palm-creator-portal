...
function getKey() {
  // WAVESPEED is the canonical env name (Evan's, 2026-07-20 key rotation);
  // WAVESPEED_API_KEY kept for older env files.
  const key = process.env.WAVESPEED_API_KEY || process.env.WAVESPEED
  if (!key) throw new Error('WAVESPEED / WAVESPEED_API_KEY is not set')
  return key
}
