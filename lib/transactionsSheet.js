...
export async function getLastFingerprints(sheets, tabName, count = 80) {
  try {
    ...
    if (totalRows <= 3) return new Set()
    const startRow = Math.max(4, totalRows - count + 1)
    const res = await withRetry(() => sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID, range: `'${tabName}'!A${startRow}:I${totalRows}`,
    }), 'lastRows.range')
