...
// The INVERSE, for reporting: an ET wall-clock sheet timestamp
// ("YYYY-MM-DD HH:mm") → the UTC calendar date (YYYY-MM-DD) it falls in. This
// is OnlyFans' statistics-graph day boundary — midnight UTC = 8 PM ET — so a
// sale at 9 PM ET on the 13th counts on the 14th, exactly like OF shows.
// DST-aware: tries EDT (+4) then EST (+5), validating the round-trip back to ET.
export function etWallToUtcDate(s) {
  ...
  if (!y || !m || !d) return ''
  const utcDateOf = (ms) => new Intl.DateTimeFormat('en-CA', { timeZone: 'UTC', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date(ms))
  for (const off of [4, 5]) {
    const ms = Date.UTC(y, m - 1, d, hh + off, mm)
    const back = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/New_York', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date(ms)).replace(',', '')
    const [bd, bt] = back.split(' ')
    if (bd === dp && bt === `${String(hh).padStart(2, '0')}:${String(mm).padStart(2, '0')}`) return utcDateOf(ms)
  }
  return utcDateOf(Date.UTC(y, m - 1, d, hh + 4, mm)) // fallback: assume EDT
}
...
// Returns a Map<fingerprint, count> of the top `count` existing rows.
// COUNTS, not a presence-Set: two legit transactions from the same fan for the
// same amount in the same MINUTE share a fingerprint (the sheet stores time to
// the minute, no seconds), so a boolean "have I seen this fp?" silently dropped
// the second one (e.g. Taby's fan Liam tipping $40 twice in one minute — $52
// vanished). Callers do a MULTISET diff instead: keep incoming rows only up to
// the count the sheet is still short by. `.has(fp)` still works for the webhook
// (true when count ≥ 1). Rows are NEWEST-first; reading the TOP `count` covers
// any realistic overlap window (cutoff − 4d) — reading the bottom once let every
// pulled row pass and duplicate the HTML originals (fixed 2026-07-03).
export async function getLastFingerprints(sheets, tabName, count = 400) {
  try {
    ...
    for (const r of (res.data.values || [])) {
      const fp = txnFingerprint(r[0], r[3], r[5])
...
// Per-account OF API state (2026-07-17): each OnlyFans Revenue Account now
// carries its own 'OF API Connect' decision (Connect|Skip) + 'OF API Account
// ID'. Pull/backfill MUST use the per-account id — the old "VIP name → second
// id in the ops comma list, else first" fallback silently pulled the FREE
// account's data into a VIP tab when the VIP page wasn't connected.
// Returns [{ name, status, connect, acctId }] for Active accounts of this AKA;
// [] on any error (callers fall back to legacy behavior).
export async function fetchRevenueAccountsApiState(creatorAka) {
  try {
    const params = new URLSearchParams()
    params.set('filterByFormula', `AND(FIND("${String(creatorAka).replace(/"/g, '\\"')} - ", {Account Name}) = 1, {Platform}="OnlyFans", {Status}="Active")`)
    for (const f of ['fldkEi3jW9tUXSTc5', 'fldbj2l06bvN5OaSR', 'fld5SXqq3V7mUi5F0']) params.append('fields[]', f)
    ...
    return (data.records || []).map((r) => {
      const f = r.fields || {}
      const connect = f['fldbj2l06bvN5OaSR']
      return {
        name: f['fldkEi3jW9tUXSTc5'] || '',
        connect: typeof connect === 'string' ? connect : connect?.name || '',
        acctId: (f['fld5SXqq3V7mUi5F0'] || '').trim(),
      }
    }).filter((a) => a.name)
  } catch { return [] }
}
