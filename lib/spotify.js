...
// Top 50 USA chart — uses Spotify's top tracks via search for popular recent tracks
// Spotify editorial playlists (37i9dQ...) require user auth, so we use the
// "Today's Top Hits" public playlist or fall back to search-based approach
let top50Cache = null
let top50CacheExpiry = 0

// Known public playlists that work with Client Credentials
const CHART_PLAYLISTS = [
  '37i9dQZEVXbLRQDuF5jeBp', // Top 50 - USA (official Spotify chart)
  '37i9dQZF1DXcBWIGoYBM5M', // Today's Top Hits (fallback)
]

export async function getTop50USA() {
  if (top50Cache && Date.now() < top50CacheExpiry) return top50Cache

  // Try each playlist until one works
  for (const playlistId of CHART_PLAYLISTS) {
    try {
      const data = await spotifyFetch(`/playlists/${playlistId}/tracks`, {
        limit: 50,
        fields: 'items(track(id,name,artists,album(name,images),duration_ms,preview_url,external_urls))',
      })
      ...
      if (tracks.length > 0) {
        top50Cache = tracks
        top50CacheExpiry = Date.now() + 60 * 60 * 1000 // 1 hour
        return tracks
      }
    } catch (err) {
      console.warn(`[Spotify] Playlist ${playlistId} failed: ${err.message}`)
    }
  }

  throw new Error('Could not access any chart playlist')
}
