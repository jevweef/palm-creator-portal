...
export async function getTop50USA() {
  ...
  // Fetch Billboard Hot 100 — use most recent Saturday (chart release day)
  const now = new Date()
  ...
  const chartDate = lastSat.toISOString().split('T')[0]

  const billboardRes = await fetch(`https://billboard-api2.p.rapidapi.com/hot-100?date=${chartDate}&range=1-100`, {
    headers: {
  ...
  if (entries.length === 0) throw new Error('Billboard returned no chart entries')

  // Enrich each track with Spotify data — 25 at a time in parallel
  const tracks = []
