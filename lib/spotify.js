...
// Give a specific reason when the input isn't a playlist link, so the user
// knows exactly what to paste instead.
function playlistParseError(input) {
  const s = String(input || '')
  if (/spotify\.com\/user\//i.test(s) || /spotify:user:/i.test(s)) {
    // Spotify blocks apps from reading a profile's playlists without that user
    // logging in, so a /user/ link can never work here — needs a playlist link.
    return 'That’s a Spotify profile link — Spotify won’t let us read a profile’s playlists. Open the specific playlist you want, tap Share → Copy link, and paste that (it looks like open.spotify.com/playlist/…).'
  }
  if (/spotify\.com\/(artist|album|track)\//i.test(s)) {
    const kind = s.match(/spotify\.com\/(artist|album|track)\//i)[1]
    return `That’s a Spotify ${kind} link, not a playlist. Paste a playlist link (open.spotify.com/playlist/…).`
  }
  return 'Could not read a Spotify playlist from that. Paste a playlist link (open.spotify.com/playlist/…) or its 22-character ID.'
}
...
// Get all tracks from a Spotify playlist URL/ID.
export async function getPlaylistTracks(input) {
  const playlistId = parsePlaylistId(input)
  if (!playlistId) throw new Error(playlistParseError(input))
  return fetchPlaylistTracksById(playlistId)
}

// Get all tracks for a specific playlist ID (API first, embed-scrape on 403).
