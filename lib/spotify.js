...
export async function findSimilarMusic(seedTracks, options = {}) {
  ...
  const artistCount = limit >= 80 ? 15 : limit >= 40 ? 10 : 5
  const perArtist = limit >= 80 ? 15 : limit >= 40 ? 10 : 6
  const genreCount = limit >= 80 ? 8 : limit >= 40 ? 6 : 4
  const perGenre = limit >= 80 ? 15 : limit >= 40 ? 10 : 6
  ...
  for (const artist of artists.slice(0, artistCount)) {
    try {
      const data = await spotifyFetch('/search', { q: `artist:"${artist}"`, type: 'track', limit: perArtist })
  ...
  const genres = [...new Set(seedTracks.flatMap(t => t.genres || []))].slice(0, genreCount)
  for (const genre of genres) {
    try {
      const data = await spotifyFetch('/search', { q: `genre:"${genre}"`, type: 'track', limit: perGenre })
      const bucket = []
